import { StdioTransport } from '@reaatech/mcp-load-test-client';
...
describe('StdioTransport', () => {
  ...
  it('should send request and receive response', async () => {
    ...
    cp.stdout.emit(
      'data',
      Buffer.from(`${JSON.stringify({ jsonrpc: '2.0', id: 1, result: { initialized: true } })}\n`),
    );
  ...
  it('should handle JSON-RPC error response', async () => {
    ...
    cp.stdout.emit(
      'data',
      Buffer.from(
        `${JSON.stringify({ jsonrpc: '2.0', id: 1, error: { code: -1, message: 'Error' } })}\n`,
      ),
    );
