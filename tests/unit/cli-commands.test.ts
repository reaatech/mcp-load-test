...
import { registerCompareCommand } from '../../src/cli/commands/compare.command.js';
  ...
  describe('registerCompareCommand', () => {
    ...
    it('should compare two reports', async () => {
      ...
      const baseline = {
        ...
        grade: 'B',
        breakingPoint: null,
        toolLatencies: [{ toolName: 'echo', latency: { p50: 10, p90: 20, p95: 25, p99: 30, min: 5, max: 40, mean: 15, samples: 100 } }],
      ...
      const current = {
        ...
        id: 'curr',
        grade: 'A',
        toolLatencies: [{ toolName: 'echo', latency: { p50: 5, p90: 10, p95: 12, p99: 15, min: 2, max: 20, mean: 8, samples: 100 } }],
      };
    ...
    it('should report unchanged grade', async () => {
      ...
      await program.parseAsync(['node', 'test', 'compare', '--baseline', baselinePath, '--current', currentPath]);
    ...
    it('should handle tools missing in baseline', async () => {
      ...
      const current = {
        ...
        id: 'curr',
        toolLatencies: [
          { toolName: 'echo', latency: { p50: 10, p90: 20, p95: 25, p99: 30, min: 5, max: 40, mean: 15, samples: 100 } },
          { toolName: 'new-tool', latency: { p50: 5, p90: 10, p95: 12, p99: 15, min: 2, max: 20, mean: 8, samples: 100 } },
        ],
      };
      ...
      const output = consoleSpy.mock.calls[0]?.[0] as string;
      const parsed = JSON.parse(output);
      const newTool = parsed.baselineComparison.toolLatencyChanges.find((t: { toolName: string }) => t.toolName === 'new-tool');
