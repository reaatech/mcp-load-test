...
import type { ToolDefinition, MCPClient, TransportType, AuthOptions } from '../types/domain.js';
      ...
      case 'oauth':
        if (this.options.auth.oauthClientId && this.options.auth.oauthClientSecret) {
          headers['Authorization'] =
            `Basic ${Buffer.from(`${this.options.auth.oauthClientId}:${this.options.auth.oauthClientSecret}`).toString('base64')}`;
