...
import { ConsoleReporter, MarkdownReporter, JsonReporter } from '../../reporters/index.js';
import { loadConfigFile } from '../config-loader.js';
import { logger } from '../../observability/logger.js';
import type { Command } from 'commander';
import type { LoadEngineOptions, LoadProfile, ToolCallPattern } from '../../types/domain.js';
import { BUILT_IN_PATTERNS } from '../../patterns/index.js';
...
export async function buildAndRun(options: RunOptions): Promise<void> {
  ...
  const onAbort = () => {
    if (!aborted) {
      ...
      engineOptions.profile = { type: 'ramp', minConcurrency: 0, maxConcurrency: 0, rampDurationMs: 0, holdDurationMs: 0 };
    }
  };
...
function buildOptions(
  ...
  const patternNames = (cliOptions.patterns || 'explore-then-act')
    .split(',')
    .map((p) => p.trim());
  ...
  return {
    ...
    outputFormat: (cliOptions.format as 'console' | 'markdown' | 'json') ?? config.outputFormat ?? 'console',
  };
}
...
function resolvePatterns(names: string[], configPatterns?: ToolCallPattern[]): ToolCallPattern[] {
  ...
  if (resolved.length === 0) {
    resolved.push(BUILT_IN_PATTERNS[0]!);
  }
...
function buildProfile(cliOptions: RunOptions, configProfile?: LoadProfile): LoadProfile {
  ...
  const maxConcurrency = parseInt(cliOptions.maxConcurrency, 10);
  const durationMs = parseInt(cliOptions.duration, 10) * 1000;
    ...
    case 'spike': {
      const baseline =
        cliOptions.baselineConcurrency !== undefined
          ? parseInt(cliOptions.baselineConcurrency, 10)
